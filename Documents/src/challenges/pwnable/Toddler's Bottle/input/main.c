...
int main(int argc, char* argv[], char* envp[]){
        ...
        if(memcmp(buf, "\x00\x0a\x02\xff", 4)) {
				printf("wrong input form stderr: %s\n", buf);
				return 0;};
        printf("Stage 2 clear!\n");
        // env
        if(strcmp("\xca\xfe\xba\xbe", getenv("\xde\xad\xbe\xef"))) {
				printf("wrong env var: %x\n" ,getenv("\xde\xad\xbe\xef"));
				return 0;};
        ...
        if( fread(buf, 4, 1, fp)!=1 ) {
				printf("fread returned wrong: ");
				FILE* temp = fopen("\x0a", "r");
				printf("%d\n", fread(buf, 4, 1, temp));
				return 0;}
        if( memcmp(buf, "\x00\x00\x00\x00", 4) ){ 
				printf("buffer is not all null %x", buf);
				return 0;}
        fclose(fp);
        ...
        if( recv(cd, buf, 4, 0) != 4 ) {
				printf("incorrect length of msg\n");
				return 0;}
